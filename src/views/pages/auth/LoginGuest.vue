...
import { LOADING_SPINNER_SHOW_MUTATION, LOGIN_ACTION_GUEST, LOGOUT_ACTION, IS_USER_AUTHENTICATE_GETTER, GET_USER_DATA_GETTER, SIGNUP_ACTION } from '@/store/storeconstants';
...
<template>
    <div class="" style="background-image: url('/img/building/nbte-16.jpeg'); background-size: cover; background-position: center; background-repeat: no-repeat">
        <div class="overlay-4 overlay-3 flex align-items-center justify-content-center min-h-screen min-w-screen overflow-hidden">
            <div
                class="flex flex-column px-5 py-5 gap-2 montserrat-font shadow-8"
                ...
                <form @submit.prevent="onLogin" v-if="showLogingForm">
                    ...
                    <div class="p-fluid">
                        <div class="formgrid grid">
                            <div class="col-12 pb-4">
                                <label for="email" class="text-primary-50 font-semibold block mb-2">Email</label>
                                <InputText id="email" type="email" placeholder="Email address" class="w-full" style="padding: 1rem" v-model="email" />
                                <small class="text-red-600" v-if="errors.email">{{ errors.email }}</small>
                            </div>
                            <div class="col-12 pb-4">
                                <label for="password" class="text-primary-50 font-semibold block mb-2">Password</label>
