...
const goToBooking = () => {
  // Navigate to the profile page
  router.push('/booking'); // Adjust the path to your profile page
};
...
<template>
  <div class=""
    style="background-image: url('/img/building/nbte-16.jpeg'); background-size: cover; background-position: center; background-repeat: no-repeat;">
    <div
      class="overlay-4 overlay-3 flex align-items-center justify-content-center min-h-screen min-w-screen overflow-hidden">
      <div class="flex flex-column px-5 py-5 gap-2 montserrat-font shadow-8"
        style="border-radius: 12px; background-image: radial-gradient(circle at left top, var(--primary-400), var(--primary-700)); width: 40vw;"
        :pt="{
          root: 'border-none',
          mask: {
        ...
        <div class="surface-section px-4 py-8 md:px-6 lg:px-8 card" v-if="showWelcomeMessage">
          <div class="text-700 text-center">
            ...
            <Button label="Explore" class="font-bold px-8 py-3 p-button-raised p-button-rounded white-space-nowrap"
        ...
        <form @submit.prevent="onSignup()" v-if="showRegForm">
            ...
            <div class="formgrid grid">
              <div class="col-12 pb-4">
                ...
                <InputText id="name" v-model.trim="name" type="text"
                  class="bg-white-alpha-40 border-none p-3 text-primary-50" placeholder="Enter username">
                </InputText>
              ...
              <div class="xl:col-6 md:col-6 lg:col-6 col-12 pb-4">
                ...
                <InputText id="password" v-model.trim="password"
                  class="bg-white-alpha-40 border-none p-3 text-primary-50" placeholder="Enter password"
                  type="password"></InputText>
                <small class="text-red-600" v-if="errors.password">{{ errors.password }}</small>
              </div>
              <div class="xl:col-6 md:col-6 lg:col-6 col-12 pb-4">
                <label for="password_confirmation" class="text-primary-50 font-semibold block mb-2">Confirm
                  Password</label>
                <InputText id="password_confirmation" v-model.trim="password_confirmation"
                  class="bg-white-alpha-40 border-none p-3 text-primary-50" placeholder="Enter confirm password"
                  type="password"></InputText>
          ...
          <div class="flex align-items-center gap-3">
            <Button label="Go to Booking" @click="goToBooking(); clearError()" text
              icon="pi pi-arrow-left" class="p-3 p-button-raised w-full text-primary-50 border-1 border-white-alpha-30 hover:bg-white-alpha-10 bg-white-alpha-20"></Button>
        ...
        <form @submit.prevent="onLogin" v-if="showLogingForm">
          ...
          <Message v-if="error" severity="error">{{ error }}</Message>
          <Message v-if="showSuccessAlert" severity="success">Your account has been created successfully. Welcome aboard!</Message>
          <div class="p-fluid">
            <div class="formgrid grid">
              <div class="col-12 pb-4">
                <label for="email" class="text-primary-50 font-semibold block mb-2">Email</label>
