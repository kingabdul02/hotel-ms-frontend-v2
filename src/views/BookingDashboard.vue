...
watch(searchQuery, () => {
    if (searchQuery.value) {
        filteredBookings.value = recentBookings.value.filter((booking) => booking.user?.name?.toLowerCase().includes(searchQuery.value.toLowerCase()) || booking.room?.name?.toLowerCase().includes(searchQuery.value.toLowerCase()));
    } else {
...
<template>
    ...
    <div class="grid mb-3">
        <div class="col-12">
            <Card class="bookings-table-card">
                ...
                <template #content>
                    <div class="table-header mb-4">
                        <InputText v-model="searchQuery" placeholder="Search bookings..." class="search-input w-full md:w-30rem">
                            <template #prepend>
                    ...
                    >
                        ...
                        <Column field="payment_status" header="Payment Status" sortable>
                            <template #body="slotProps">
                                <Tag :value="slotProps.data.payment_status === 'paid' ? 'Paid' : 'Pending'" :severity="slotProps.data.payment_status === 'paid' ? 'success' : 'warning'" />
                            </template>
                        </Column>

                        <Column header="Actions">
                            <template #body="slotProps">
                                <Button label="View Details" icon="pi pi-eye" class="p-button-sm p-button-outlined" @click="showDetails(slotProps.data)" />
                            </template>
                        </Column>
                    </DataTable>
                </template>
            ...
            <BookingDetailsDialog :visible="showDialog" :selectedBooking="selectedBooking" @update:visible="showDialog = $event" @update="statisticsBooking" />
        </div>
    </div>
</template>
