...
export function useCorporateBooking() {
    ...
    const fetchBookingById = async (bookingId) => {
        ...
        try {
            ...
            corporateBookingForm.halls = booking.halls
    ...
    const searchCompanies = (event) => {
        filteredCompanies.value = companies.value.filter((company) => company.name.toLowerCase().includes(event.query.toLowerCase()));
    };
    ...
    const checkInGuest = async (guest) => {
        ...
        try {
            ...
            await axiosInstance.put(
                `/admin/corporate-booking/guest/${guest.id}/check-in`,
                {},
                {
                    headers: { Authorization: `Bearer ${token}` }
                }
    ...
    const checkOutGuest = async (guest) => {
        ...
        try {
            store.commit(LOADING_SPINNER_SHOW_MUTATION, true);
            await axiosInstance.put(
                `/admin/corporate-booking/guest/${guest.id}/check-out`,
                {},
                {
                    headers: { Authorization: `Bearer ${token}` }
                }
    ...
    const fetchCorporateBill = async (reservationCode) => {
        ...
        try {
            store.commit(LOADING_SPINNER_SHOW_MUTATION, true);
            const response = await axiosInstance.get(`admin/corporate-booking/bill/${reservationCode}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
